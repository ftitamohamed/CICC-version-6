...
let slug1 = 'XYU5562157';
localStorage.setItem('selectedSlug', slug1);
...
function formatText(command, value = null) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
      fetch(`https://custmize.digitalgo.net/api/get_single_product/${slug}`)
...
document.getElementById('downloadBtn').addEventListener('click', downloadImage);
 async function addProducts(){
    ...
    productsDiv.classList='productDiv';
    let images = JSON.parse(localStorage.getItem('productsImages'));
    let slugs = JSON.parse(localStorage.getItem('productsSlugs'));
    ...
    images.forEach((image,index)=>{
        ...
        img.addEventListener('click', () => {
            ...
            localStorage.setItem('selectedSlug', slugs[index]);
            console.log('Clicked slug saved:', slug);
            img.classList.add('inside');
            setTimeout(()=>{window.location.href = 'boxProductCustomize.html';
            console.log('Clicked ',img.classList);
        },500);  
        });
    });
