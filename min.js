document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", async (event) => {
            ...
            try {
                ...
                const response = await fetch("https://dashboard.caduksa.com/api/login", {
                    method: "POST",
                    headers: {
                ...
                if (data.success) {
                    ...
                    window.location.href = "wearHouseCustomize.html"; // Adjust as needed
                } else {
    ...
    if (signUpForm) {
        signUpForm.addEventListener("submit", async (event) => {
            ...
            const name = signUpForm.querySelector('input[placeholder="user name"]').value.trim();
            const email = signUpForm.querySelector('input[placeholder="e-mail"]').value.trim();
            ...
            const phone = signUpForm.querySelector('input[placeholder="phone number" ]').value.trim();
            ...
            if (password !== confirmPassword) {
                showErrorMessage(signUpForm, " Passwords are not identical  ");
                return;
            }

            try {
                document.getElementById("loader").style.display = "inline-block";
                const response = await fetch("https://dashboard.caduksa.com/api/register", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Accept-Language": "ar",
