...
canvas.on('object:scaling', function (e) {
    ...
    // 🔥 Get real displayed size
    const scaledWidth = obj.getScaledWidth();
    const scaledHeight = obj.getScaledHeight();

    if (scaledWidth < MIN_SIZE || scaledHeight < MIN_SIZE) {

        const scaleX = MIN_SIZE / obj.width;
        const scaleY = MIN_SIZE / obj.height;
        ...
        obj.setCoords(); // 🔥 VERY IMPORTANT for images
    }
    if (obj.type === 'image') {
        ...
        obj.setCoords(); // important for stability
    }
    calculateAndUpdatePrice(e.target);
});
      ...
      addTextButton.addEventListener('click', () => {
          ...
          if (styledHTML) {
              ...
              Array.from(tempDiv.childNodes).forEach((node) => {
                ...
                // 🔥 Target size = 3 cm in pixels
                const CM_TO_PX = 10;
                const targetSize = 3 * CM_TO_PX; // ≈ 113.4 px
