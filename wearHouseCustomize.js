...
// Alternative async/await version of the function
async function setGuidnessImagesAsync(slug) {
    try {
        const response = await fetch(`https://dashboard.caduksa.com/api/single-external-product/${slug}`);
...
// Then in your DOMContentLoaded, use:
// const guidness_pictures = await setGuidnessImagesAsync(slug);
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const guidness_pictures = await setGuidnessImagesAsync(slug);
    console.log('this is the guidness image array ',guidness_pictures);
 ...
 */
 function openPreviewSlider(images, startImage) {
    ...
    let currentIndex = images.indexOf(startImage);
    ...
    modal.appendChild(sliderWrapper);
...
function formatText(command, value = null) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
      fetch(`https://dashboard.caduksa.com/api/single-external-product/${slug}`)
  ...
  function updatePriceDisplay() {
    ...
    if(!flag){
        ...
        localStorage.setItem('unitPrice',adjustedMainPrice);
        ...
        // Calculate the total price piecePrice
        const totalPrice = adjustedMainPrice * countNumber;
...
function fetchPriceRanges() {
    return fetch('https://dashboard.caduksa.com/api/size_calculate_new_factor')
        .then(response => response.json())
        .then(data => {
...
function calculateAndUpdatePrice(activeObject) {
    if (!activeObject || !priceRanges) return;
...
/* canvas.on('object:scaling', (e) => {
    calculateAndUpdatePrice(e.target);
});
 */
...
// ✅ Correct conversion
const CM_TO_PX = 10; // or 300 / 2.54 for print
const MIN_SIZE = 3 * CM_TO_PX;
...
canvas.on('object:scaling', function (e) {
    ...
    if (scaledWidth < MIN_SIZE) { 
        obj.scaleX = MIN_SIZE / obj.width + 0.03 ;
    }

    // 🔥 Enforce minimum height
    if (scaledHeight < MIN_SIZE) {
        obj.scaleY = MIN_SIZE / obj.height +0.03 ;
    }

    // 🔥 Special handling for images (keep your logic)
    if (obj.type === 'image') {
        if (obj.scaleX < obj.minScaleX) {
            obj.scaleX = obj.minScaleX;
        }
        if (obj.scaleY < obj.minScaleY) {
            obj.scaleY = obj.minScaleY;
        }
    }

    obj.setCoords(); // VERY important

    calculateAndUpdatePrice(obj);
});
       ...
       item.addEventListener("click", (event) => {
           ...
           imageElement.onload = function () {
            ...
            image.set({
                ...
                scaleY: 0.3,
                scaleX: 0.3,
                lockScalingFlip: true, // optional but recommended
            });
        
            // 🔥 Save initial scale as minimum
            image.minScaleX = image.scaleX;
            image.minScaleY = image.scaleY;
        
            // Assign a unique ID for tracking prices
            image.id = `img_${Date.now()}`;
            logoPrices.set(image.id, 3);
   ...
   imageInput.addEventListener("change", (e) => {
       ...
       if (imgObj) {
           ...
           reader.onload = (e) => {
               ...
               img.onload = function() {
                   // Calculate scale to make the image 30x30 pixels
                   const targetSize = 30;
                   ...
                   fabricImage.id = `img_${Date.now()}`;
                   logoPrices.set(fabricImage.id, 3);
                   // Stretch to exact 30x30
                    const scaleX = 40 / img.width;
                    const scaleY = 40 / img.height;
                    ...
                    // 🔥 Save initial scale as minimum
                    fabricImage.minScaleX = fabricImage.scaleX;
                    fabricImage.minScaleY = fabricImage.scaleY;
          ...
          if (styledHTML) {
              ...
              Array.from(tempDiv.childNodes).forEach((node) => {
                  ...
                  const textObject = new fabric.Text(node.textContent.trim(), {
                    ...
                    const CM_TO_PX = 10; // screen
                    // OR for print accuracy:
                    // const CM_TO_PX = 300 / 2.54;

                    const targetSize = 3 * CM_TO_PX;

                    const scaleX = targetSize / textObject.width;
                    const scaleY = targetSize / textObject.height;
                    const scale = Math.min(scaleX, scaleY);

                    textObject.scaleX = targetSize / textObject.width;
                    textObject.scaleY = targetSize / textObject.height;
...
canvas.on('selection:created', (e) => {
    if (e.selected[0]?.type === 'text') {
        ...
        document.querySelector('.editor').innerText = activeTextObject.text;
    }
});
...
editor.addEventListener('input', () => {
    ...
    activeTextObject.setCoords(); // VERY important
    canvas.renderAll();

    calculateAndUpdatePrice(activeTextObject);
});
...
deleteButton.addEventListener("click", () => {
    ...
    if (activeObject) {
        ...
        toggelersArray.forEach(toggler => {
            if (toggler.classList.contains('toggelers-active')) {
                toggler.click();
 ...
 */
    return dataURL;
        ...
        async function extractLogosAndText() {
            ...
            for (const toggler of toggelers) {
                const canvasJSON = sessionStorage.getItem(`${toggler.textContent}`);
            ...
            console.log('the saved sizes and logos from the root function are ',allTogglersData);
 ...
 async  function  saveToLocalStorage() {
    ...
    const toggelers = document.querySelectorAll('.toggelers p');
      ...
      console.log('the sizes and saves logos are ',allTogglersData);
      const toggelersArray = Array.from(toggelers);
      toggelersArray[0].click();
       ...
       /*  tshirtImages = JSON.parse(localStorage.getItem('tshirtImages')) || []; */
        ...
        let delivery = JSON.parse(localStorage.getItem('deliveryDate'));
        console.log('this is the delivery date from the customize page ',delivery); 
       ...
       /*  if(!token){
        ...
        logCartData();
        updateCartCount();
        document.getElementById('saveLogos').style.display = 'block';
...
document.getElementById("preview").addEventListener("click", async () => {
    const toggelers = Array.from(
        document.querySelectorAll(".toggelers p")
    ...
    try {
        ...
        loadingOverlay.id = "preview-overlay";
        ...
        for (const toggler of toggelers) {
            ...
            images.push({
                ...
                label: toggler.textContent.trim(),
            });
        }
        ...
        previewContainer.id = "preview-gallery";
        ...
        images.forEach((imageData) => {
            ...
            badge.textContent = imageData.label;
...
async function saveLogos() {
    ...
    document.getElementById('saveLogos').addEventListener('click', saveLogos);
