...
async function fetchAndsave(slug){
  const productUrl= `https://custmize.digitalgo.net/api/single-external-product/${slug}`
  fetch(productUrl)
      .then(response => {
          if (!response.ok) {
              throw new Error(`HTTP error! status: ${response.status}`);
    ...
    function  saveToLocalStorage(product) {
      const newOrder =  {
          ...
          size_id: product.colors?.[0].sizes?.[0]?.id  || 'None',
          quantity: product.count,
          default_code:product.default_codes?.[0] || 'None',
          external_id:product.external_id || 'None',
          price_without_size_color_price: product.price,
          price_for_size_color_price: product.price,
          ...
          back_image: {
              url: product.colors?.[0]?.back_image||  "",
              logos:  []
          },
          right_side_image: {
              url: product.colors?.[0]?.right_side_image||  "",
              logos:  []
          },
          left_side_image: {
              url: product.colors?.[0]?.left_side_image||  "",
              logos:  []
          }
      };
    ...
    const newOrder1 = {
      ...
      size_id: product.colors?.[0]?.sizes?.[0]?.id || 'M',
      quantity: product.count,
