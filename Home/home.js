...
document.getElementById('saveTrackerCodeButton').addEventListener('click', function () {
    ...
    if (wantedProduct) {
        ...
        alert('Enter the product name please');
    }
});

document.querySelector('body > header > section > div > input[type=text]').addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        const wantedProduct = document.querySelector('.trackerDropDown > div:nth-child(1) > input:nth-child(2)').value;
        if (wantedProduct) {
            localStorage.setItem('TrackerCode', wantedProduct);
            window.location.href = 'visitor.html';
        } else {
            alert('Enter the tracker code please');
        }
    }
});
...
document.getElementById('trackerCodeInput').addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        const wantedProduct = document.getElementById('trackerCodeInput').value;
    if (wantedProduct) {
        localStorage.setItem('wantedProduct', wantedProduct);
...
toggleBtn2.addEventListener("click", () => {
  ...
  console.log('the button is clicked ! ',toggleBtn2);
});
toggleBtn.addEventListener("click", () => {
  tracker.classList.toggle("show");
  console.log('the button is clicked ! ',toggleBtn);
}); */

document.querySelector('.trackerDropDown > div:nth-child(1) > img:nth-child(1)').addEventListener('click', function () {
    const wantedProduct = document.querySelector('.trackerDropDown > div:nth-child(1) > input:nth-child(2)').value;
    if (wantedProduct) {
        localStorage.setItem('TrackerCode', wantedProduct);
        window.location.href = 'visitor.html';
