...
let verified ;
let asideClassList = document.querySelector(' main aside').classList;
let btnState = document.getElementById("showAside").classList;
...
async function getUserProfile() {
    ...
    try {
        ...
        sessionStorage.setItem('otpVerify',data.data.otp);
        let asideClassList = document.querySelector(' main aside').classList;
        console.log('this is the aside classlist from otp.js',btnState);
        console.log('this is the opt button ',document.querySelector('.otpButton'))
        ...
        if (data.success && data.data.otp !== null) {
            ...
            } else if ((btnState !== 'arrow') && (window.innerWidth < 900)){
                document.querySelector('.otpButton').style.display = 'none';
                showOTPDialog(data.message, email);
            }
           
        }else if (data.data.otp === null){
            console.log('the otp value is ',data.data.otp );
            document.querySelector('.otpButton').style.display = 'none';
        }
